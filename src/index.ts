...
import {FSHADER_FOR_SUN, VSHADER_SOURCE_FOR_SUN} from "src/shaders/Sun";
import {createCamera} from "src/utilities/createCamera";
import {createComposer} from "src/utilities/createComposer";
import {createRenderer} from "src/utilities/createRenderer";
import {getDefaultUniforms} from "src/utilities/getDefaultUniforms";
import {getZFromImageDataPoint} from "src/utilities/getZFromImageDataPoint";
import {hexToRgb} from "src/utilities/hexToRgb";
import {loadImage} from "src/utilities/loadImage";
import {loadSceneBackground} from "src/utilities/loadSceneBackground";
import {runApp} from "src/utilities/runApp";
...
let camera = createCamera(70, 1, 1020, { x: 0, y: 0, z: 2.4 })
...
let composer = createComposer(renderer, scene, camera, (comp: EffectComposer) => {
...
let app = {
  ...
  async initScene() {
    ...
    // second loop we load the HeightMap data horizontally inverse
    for (let i = 0; i < 2; i++) {
      ...
      for (let index = 0; index < geometryUVs.length / 2; index++) {
        ...
        geometryPositions[index * 3 + 2] = getZFromImageDataPoint(hm_imageData, (i === 0 ? vertexU : 1 - vertexU), vertexV, canvas.width, canvas.height)
      }
    ...
    const sunMat = new THREE.ShaderMaterial({
      uniforms: uniforms,
      vertexShader: VSHADER_SOURCE_FOR_SUN,
      fragmentShader: FSHADER_FOR_SUN,
      transparent: true
    })
    ...
    gui.add(params, "speed", 0, 10, 0.5).name('Plane speed')
  ...
  updateScene(interval:number, elapsed: number) {
